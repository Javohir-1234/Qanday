...
async def check_sub_callback(callback: CallbackQuery, state: FSMContext):
    if await is_subscribed(callback.from_user.id):
        ...
        # Siz so'ragandek, obuna tasdiqlanganda to'liq start xabari chiqadi
        welcome_text = (
            ...
            f"━━━━━━━━━━━━━━━━━━\n"
            f"⚡ 30 soniyalik — <b>{SONG_PRICE_SHORT:,} so'm</b>\n"
            f"🎶 2-3 daqiqalik — <b>{SONG_PRICE_FULL:,} so'm</b>\n"
...
async def create_song_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"⚡ <b>30 soniyalik</b> — {SONG_PRICE_SHORT:,} so'm\n"
        f"🎶 <b>2-3 daqiqalik</b> — {SONG_PRICE_FULL:,} so'm\n\n"
        f"💰 Sizning balansingiz: <b>{balance:,} so'm</b>",
        parse_mode="HTML",
...
async def give_money_final(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("Xato kiritildi. Summa faqat raqam bo'luk bo'lishi kerak.")
    finally:
        await state.clear()
