...
CHANNEL_USERNAME = "@QoshiqYaratishBot"
CHANNEL_LINK = "https://t.me/QoshiqYaratishBot"
...
# === CHEKNI TEKSHIRISH ===
# Qaytaradi: (success: bool, amount_or_error, photo_hash)
# success=True  => amount_or_error = int (haqiqiy summa)
# success=False => amount_or_error = str (xato xabari)

def extract_amount_from_text(text):
    if not text:
        return None
    # Avval so'm bilan yozilgan summalarni qidiradi
    patterns = [
...
async def check_receipt_photo(file_id, expected_amount):
    """
    Qaytaradi: (success, amount_or_message, photo_hash)
    success=True  => amount_or_message = int (kreditlanadigan summa)
    success=False => amount_or_message = str (xato xabari)
    photo_hash = None bo'lsa texnik xatolik
    """
    try:
        # Rasmni yuklab olish
        ...
        return False, f"❌ Texnik xatolik: {e}", None
...
async def process_receipt(message: Message, state: FSMContext):
    ...
    if success:
        ...
        actual_amount = amount_or_msg  # int
        ...
        # Admin qo'lda tekshirishi kerak
        error_msg = amount_or_msg  # str
        ...
        caption = (
            f"💳 <b>YANGI CHEK — QO'LDA TEKSHIRISH KERAK</b>\n\n"
            f"👤 {message.from_user.full_name}\n"
            f"🔗 {user_info}\n"
            f"🆔 ID: <code>{message.from_user.id}</code>\n"
        ...
        # Texnik xatolik — photo_hash yo'q
        ...
        try:
            await bot.send_photo(
                ...
                photo=file_id,
                caption=(
                    f"💳 <b>TEXNIK XATOLIK — CHEK TEKSHIRILMADI</b>\n\n"
                    f"👤 {message.from_user.full_name}\n"
                    f"🔗 {user_info}\n"
                    f"🆔 ID: <code>{message.from_user.id}</code>\n"
                    f"💰 Kutilgan summa: {expected_amount:,} so'm\n"
