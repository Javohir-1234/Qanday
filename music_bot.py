...
SONG_PRICE_1 = 10000   # 1-2 daqiqalik
SONG_PRICE_2 = 20000   # 3 daqiqalik
...
async def check_receipt_photo(file_id, expected_amount):
    ...
    try:
        ...
        if not client:
            return False, "⚠️ Vision client ishlamadi — admin qo'lda tekshiradi.", photo_hash
        ...
        if response.error.message:
            ...
            return False, "⚠️ OCR xatolik — admin qo'lda tekshiradi.", photo_hash
        ...
        if not detected_amount:
            return False, "❌ Chekdan summa topilmadi — admin qo'lda tekshiradi.", photo_hash
        ...
        try:
            photo_hash = get_image_hash(image_content) if image_content else hashlib.md5(str(file_id).encode()).hexdigest()
        except Exception:
            photo_hash = hashlib.md5(str(file_id).encode()).hexdigest()
        return False, "⚠️ Texnik xatolik — admin qo'lda tekshiradi.", photo_hash
...
def get_song_type_menu():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text=f"🎵 1-2 daqiqalik — {SONG_PRICE_1:,} so'm")],
        [KeyboardButton(text=f"🎶 3 daqiqalik — {SONG_PRICE_2:,} so'm")]
...
async def send_start_message(chat_id, user_id, fullname, username):
    ...
    text = (
        ...
        f"📌 Narxlar:\n"
        f"🎵 1-2 daqiqalik — {SONG_PRICE_1:,} so'm\n"
        f"🎶 3 daqiqalik — {SONG_PRICE_2:,} so'm\n\n"
    )
    if is_new:
...
async def secret_code_handler(message: Message, state: FSMContext):
    ...
    if user_data and user_data[4]:
        return  # allaqachon ishlatilgan, hech narsa demaydi
...
async def deposit_cmd(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Minimal: {SONG_PRICE_1:,} so'm\n\nSummani faqat raqamlarda kiriting (masalan: 10000):",
        parse_mode="HTML", reply_markup=get_main_menu(message.from_user.id)
    )
...
async def process_deposit_amount(message: Message, state: FSMContext):
    ...
    if not message.text or not message.text.strip().isdigit():
        await message.answer("❌ Iltimos, summani faqat raqamlarda kiriting (masalan: 10000):")
        return
    amount = int(message.text.strip())
    if amount < SONG_PRICE_1:
        await message.answer(f"❌ Minimal summa {SONG_PRICE_1:,} so'm. Qayta kiriting:")
        return
...
async def process_receipt(message: Message, state: FSMContext):
    ...
    expected_amount = data.get('deposit_amount', SONG_PRICE_1)
    ...
    if success:
        ...
        caption = (
            f"💳 <b>YANGI CHEK KELDI (QO'LDA TEKSHIRISH KERAK)</b>\n\n"
            f"👤 {message.from_user.full_name}\n"
            f"🔗 {user_info}\n"
            f"🆔 ID: <code>{message.from_user.id}</code>\n"
            f"💰 Kutilgan summa: {expected_amount:,} so'm\n"
            f"📊 Oldingi depositlar: {deposit_count} ta\n"
            f"⚠️ {error_msg}\n\n"
        ...
        await message.answer(
            ...
            f"📝 Sabab: {error_msg}\n\n"
            f"👨‍💼 Chekingiz admin tomonidan tekshiriladi (24 soat ichida).\n"
            f"🔑 Chek ID: {deposit_id}\n\n"
            f"Tezroq bo'lishi uchun: {ADMIN_USERNAME}",
...
async def process_song_type(message: Message, state: FSMContext):
    if message.text in MENU_BUTTONS:
        ...
        return
    if message.text == f"🎵 1-2 daqiqalik — {SONG_PRICE_1:,} so'm":
        price, song_type = SONG_PRICE_1, "1-2 daqiqalik"
    elif message.text == f"🎶 3 daqiqalik — {SONG_PRICE_2:,} so'm":
        price, song_type = SONG_PRICE_2, "3 daqiqalik"
    else:
...
async def process_song_genre(message: Message, state: FSMContext):
    ...
    price = data.get('song_price', SONG_PRICE_1)
    user_data = db_get_user(message.from_user.id)
